function writeRHSfile(PoCETsys,filename)
% writeRHSfile(PoCETsys, 'filename') 
% writes a function-file filename.m containing the states' expanded
...
for i=1:numel(states)
 ...
 writeODE = [writeODE 'ddt_' states{i} ' = '];
 ...
 for j=1:numel(PoCETsys.ode(i).terms)
  ...
  %% states
  [~,nIDX] = find(PoCETsys.ode(i).terms(j).state_index);
  tmpIDX = PoCETsys.ode(i).terms(j).state_index(nIDX);
  tmpDEG = PoCETsys.ode(i).terms(j).state_degrees(nIDX);
  ...
  %% parameters
  [~,nIDX] = find(PoCETsys.ode(i).terms(j).param_index);
  tmpIDX = PoCETsys.ode(i).terms(j).param_index(nIDX);
  tmpDEG = PoCETsys.ode(i).terms(j).param_degrees(nIDX);
  ...
  %% inputs
  [~,nIDX] = find(PoCETsys.ode(i).terms(j).input_index);
  tmpIDX = PoCETsys.ode(i).terms(j).input_index(nIDX);
  tmpDEG = PoCETsys.ode(i).terms(j).input_degrees(nIDX);
  ...
  writeODE = [writeODE num2str(PoCETsys.ode(i).terms(j).coefficient) ...
...
PoCETode = ...
 sprintf('function dXdt = %s(t,X,PoCETsys%s)\n M = PoCETsys.coeff_matrices;\n%send',...
  filename(1:end-2),writeIPAR,...
  [writeIC writeIFUN writeKRON writeODE writeDXDT]);
